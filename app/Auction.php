...
//        dd($tempData);
        ...
        foreach ($tempData as $k=>$o){
            foreach ($o as $type=>$auctions){
                foreach ($auctions as $auction){
                    ...
                    if (Carbon::createFromFormat('Y-m-d H:i:s',$start)->toDateString()==Carbon::createFromFormat('Y-m-d H:i:s',$end)->toDateString()){
                        $auctionsData[$k][$type]['auctions'][$auction->type_id]['id']=$auction->id;
                    }
    ...
    public static function updateAuction($request, $id)
    {
        $office_id=$request->input('office_id');
        $type=$request->input('type');
        $type_id=$request->input('type_id');
