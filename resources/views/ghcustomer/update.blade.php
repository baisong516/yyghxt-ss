        ...
        <div class="box-header">
            ...
            <div class="box-tools">
                <div class="input-group input-group-sm" style="width: 80px;">
                    @ability('superadministrator', 'update-gh_customers')
                    <a href="{{route('ghcustomers.index')}}" class="btn-sm btn-info">返回</a>
                    @endability
                </div>
            </div>
        </div>
        <form action="{{route('ghcustomers.update',$customer->id)}}" method="post" class="ghcustomers-form form-horizontal">
            {{csrf_field()}}
            {{method_field('PUT')}}
            @include('ghcustomer.form')
        </form>
    </div>
@endsection
