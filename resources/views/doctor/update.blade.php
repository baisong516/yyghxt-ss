        ...
        <div class="box-header">
            ...
            <div class="box-tools">
                <div class="input-group input-group-sm" style="width: 80px;">
                    @ability('superadministrator', 'update-doctors')
                        <a href="{{route('doctors.index')}}" class="btn-sm btn-info">返回</a>
                    @endability
                </div>
            </div>
        </div>
        <form action="{{route('doctors.update',$doctor->id)}}" method="post" class="doctors-form form-horizontal">
            {{csrf_field()}}
            {{method_field('PUT')}}
            @include('doctor.form')
        </form>
