<style>
    ...
    <div class="form-group {{empty($errors->first('tel'))?'':'has-error'}}" style="margin-left: 20px;">
        ...
        <input type="text" name="tel" maxlength="11" value="{{isset($customer)?\App\Aiden::phoneCheck($customer->tel,$customer->customer_condition_id,$enableViewPhone,$isAdmin,3,$customer->user_id,$customer->arrive_at):old('tel')}}"
    ...
    <div class="form-group {{empty($errors->first('wechat'))?'':'has-error'}}" style="margin-left: 20px;">
        ...
        <input type="text" name="wechat" class="form-control" value="{{isset($customer)?\App\Aiden::wechatCheck($customer->wechat,$customer->customer_condition_id,$enableViewWechat,$isAdmin,3,$customer->user_id,$customer->arrive_at):old('wechat')}}" id="qq" placeholder="{{empty($errors->first('wechat'))?'微信':$errors->first('wechat')}}"
        @if(isset($customer))
             @if(!$isAdmin)
                @if(in_array($customer->customer_condition_id,[1,2,5])||$customer->user_id!=$userid)
                    disabled
