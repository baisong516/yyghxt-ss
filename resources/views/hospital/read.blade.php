    ...
    <div class="box box-info">
            ...
            <div class="box-tools">
                <div class="input-group input-group-sm" style="width: 80px;">
                    @ability('superadministrator', 'create-hospitals')
                        <a href="{{route('hospitals.create')}}" class="btn-sm btn-info">新增医院</a>
                    @endability
                </div>
            </div>
        </div>
        <div class="box-body">
            <form action="" method="post" class="hospitals-form">
                {{method_field('DELETE')}}
                {{csrf_field()}}
            <table id="hospital-list-table" class="table table-striped table-bordered table-hover">
                <thead>
                <tr>
                    <th>ID</th>
                    <th>Hospital</th>
                ...
                @foreach($hospitals as $hospital)
                    <tr>
                        ...
                        <td>
                            @ability('superadministrator', 'upade-hospitals')
                                <a href="{{route('hospitals.edit',$hospital->id)}}"  alt="编辑" title="编辑"><i class="fa fa-edit"></i></a>
                            @endability
                            @ability('superadministrator', 'delete-hospitals')
                                <a href="javascript:void(0);" data-id="{{$hospital->id}}"  alt="删除" title="删除" class="delete-operation"><i class="fa fa-trash"></i></a>
                            @endability
                        </td>
                    </tr>
                @endforeach
